...
def drawWall():
    for each in boundry:
        ...
        pygame.draw.rect(DISPLAYSURF, OUTLINE, wallRect)
        ...
        for event in pygame.event.get(): # event handling loop
            if event.type == QUIT:
                ...
                if (event.key == K_LEFT ) and direction != RIGHT:
                    ...
                    direction = DOWN
                elif (event.key == K_f) and Speed<40:
                    Speed+=1
                elif (event.key == K_s) and Speed>7:
                    Speed-=1
                elif event.key == K_ESCAPE:
                    terminate()

        # check if the Snake has hit itself or the edge
        if wormCoords[HEAD]['x'] == 0 or wormCoords[HEAD]['x'] == Cell_W-1 or wormCoords[HEAD]['y'] == 0 or wormCoords[HEAD]['y'] == Cell_H-1:
            return # game over
...
def getRandomLocation():
    return {'x': random.randint(1, Cell_W - 2), 'y': random.randint(1, Cell_H - 2)}
...
def drawWorm(wormCoords):
    for coord in wormCoords:
        ...
        pygame.draw.rect(DISPLAYSURF, SNAKE_OUT, wormSegmentRect)
        wormInnerSegmentRect = pygame.Rect(x + 4, y + 4, Cell_Size - 8, Cell_Size - 8)
        pygame.draw.rect(DISPLAYSURF, SNAKE_FILL, wormInnerSegmentRect)
